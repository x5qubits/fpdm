					...
					if(preg_match("/^(\d+) (\d+) obj/",$CurLine,$match)) {
						...
						if($obj) {
							...
							if(preg_match("/endobj/",$CurLine,$match)) {
								...
								//FIX: track object name and parent for nested field resolution
								if($name != '') {
									$_objNames[$obj]=$name;
									if($parentObjId>0) {
										$_objParents[$obj]=$parentObjId;
									}
								}
								//ENDFIX
								...
								if($subtype=="Widget") {
									...
									if($name != '') {
										//FIX: use temp key for nested fields to prevent overwriting
										if($parentObjId>0) {
											$lines["__obj_".$obj]=$object;
										} else {
											$lines["$name"]=$object;
										}
										//ENDFIX
								...
								if(array_key_exists("length",$stream)) { //length is mandatory
									...
									*/
									...
									if(($type=='')||($subtype=='')||($name=="")) {
										...
										if(($name=="")&&preg_match("/\/T\s?\((.+?)\)/",$this->_protectContentValues($CurLine),$match)) {
